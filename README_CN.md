...
- **多知识源混合检索** - FAQ 与文档候选共享内存向量索引和 SQL LIKE 关键词 fallback，由统一检索器合并、去重和排序。
...
管理后台的模型配置页可以把地址、服务商和模型名等非敏感运行时配置覆盖保存到 SQLite。API Key 不会写入 SQLite，必须通过环境变量或部署 Secret 注入。
