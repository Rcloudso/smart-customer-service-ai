...
当前版本：**v0.2.5（pre-1.0）**。在 1.0 之前，API 和持久化数据结构仍可能调整。
...
管理员可以维护 FAQ、重建 FAQ 索引、查看索引状态、调试某个问题为什么命中了某条 FAQ、查看会话记录，并在“知识审核”页面把答不好的问题沉淀成可复用知识。
...
FAQ 只是当前 MVP 的第一种知识来源，不是永久的 RAG 边界。v0.2.5 保存了通用检索快照，后续可以增加文本、网页、PDF/Word 文档切片、扫描件与截图 OCR，以及图片视觉理解，而不必重写知识审核流程。
...
- v0.2.5 只支持 FAQ 知识来源；文档上传、网页采集、OCR 和图片 RAG 尚未实现。
...
- 扩展检索评测数据集和阈值调优能力。
- 增加文档、网页、PDF/Word、OCR 和图片知识来源适配器。
- 增加后台角色权限。
