...
- Authorization is checked before idempotency replay. New grants revoke old
  grants and closed sessions revoke active grants.
...
- Routing: **9/9** fixed Chinese/English and injection cases.
...
- Tool evaluation: routing **10/10**, Demo states **4/4**, and every zero-call,
...
- `PLAYWRIGHT_CHANNEL=chromium npm run test:e2e`: **52/52 passed**, covering
  strict Cookie attributes, authorization replay binding, safe history,
