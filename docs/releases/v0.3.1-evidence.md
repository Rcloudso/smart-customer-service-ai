# v0.3.1 Release Evidence — Multimodal Knowledge Review
...
- strict worker request/result contracts, engine/version matching, response
  limits, authentication, malformed-envelope handling, fake worker
  timeouts/failures, and the local PP-StructureV3 mapper;
...
Checked locally on 2026-07-27 and 2026-07-28:

- local worker mapper/request-boundary tests: 6/6 passed;
- an isolated Python 3.10 environment installed PaddlePaddle `3.0.0`,
  PaddleOCR `3.0.3`, and PaddleX `3.0.3`; the real PP-StructureV3 CPU pipeline
  initialized and processed a repository JPEG through the authenticated HTTP
  worker with HTTP 200, 16 Blocks, one page, and no warnings;
...
Docker is not installed in the verification environment, so the image build
and `docker compose config` were not run locally. The equivalent Dockerfile
Python 3.10 dependency stack and a real Paddle model/HTTP worker request were
verified natively. The container build remains a documented deployment risk.
...
The 18-second [v0.3.1 demo video](https://github.com/Rcloudso/smart-customer-service-ai/releases/download/v0.3.1/resolveweave-v0.3.1-demo.mp4)
and repository-hosted [`v0.3.1-preview.gif`](../demo/v0.3.1-preview.gif) show
the same deterministic browser scenario end to end: inspect authoritative and
shadow evidence, edit a Block, save revision 2, publish the whole document,
then answer a customer question with page, Block, extraction-job, and engine
provenance. The browser capture uses fixed project-contract OCR responses for
repeatability; the separate native worker verification above is the evidence
that a real Paddle pipeline and authenticated worker boundary completed.

| Engine comparison | Block review | Published knowledge | Chat provenance |
| --- | --- | --- | --- |
| ![Paddle authoritative and DeepSeek shadow agreement](assets/v0.3.1-ocr-comparison.jpg) | ![OCR Block editor with visible save and publish actions](assets/v0.3.1-block-review.jpg) | ![Reviewed image document published as searchable knowledge](assets/v0.3.1-published-document.jpg) | ![Customer answer showing OCR page, Block, job and engine provenance](assets/v0.3.1-chat-provenance.jpg) |
...
- PaddleOCR and PaddleX are intentionally pinned to the matching `3.0.3`
  release because an unconstrained newer PaddleX fails pipeline initialization.
